...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
            ...
            InlineKeyboardButton('✨ᴜᴘᴅᴀᴛᴇꜱ ᴄʜᴀɴɴᴇʟ✨', url="https://t.me/Galaxy_bots1"),
            InlineKeyboardButton('✨ᴍᴏᴠɪᴇꜱ Gʀᴏᴜᴘ✨', url='https://t.me/MoviesGalaxy120')
        ]]
    ...
    if len(message.command) != 2:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_text("<i>ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ <b>ɢᴀʟᴀxʏ ғɪʟᴛᴇʀ ʙᴏᴛ</b>.\nʜᴏᴘᴇ ʏᴏᴜ'ʀᴇ ᴅᴏɪɴɢ ᴡᴇʟʟ...</i>")
    ...
    if data.split("-", 1)[0] == "MV":
        ...
        if vj and PREMIUM_AND_REFERAL_MODE == True:
            ...
            reply_markup = InlineKeyboardMarkup(buttons)
            m=await message.reply_sticker("CAACAgIAAxkBAAENCn5md71o7gTUPaxJ9w1nCBcwHf9FcAACxwIAAs9fiwdxXUx0SM7_lR4E") 
            await asyncio.sleep(1)
            await m.delete()
            await message.reply_photo(
                photo=random.choice(PICS),
            ...
            try:
                ...
                if STREAM_MODE == True:
                    button = [[
                        ...
                        InlineKeyboardButton('𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥', url="https://t.me/Ek4mpreetsingh")
                    ],[
                    ...
                    button = [[
                        ...
                        InlineKeyboardButton('✨ᴏᴡɴᴇʀ✨', url="https://t.me/Ek4mpreetsingh")
                    ]]
        ...
        for file in files:
            ...
            if STREAM_MODE == True:
                button = [[
                    ...
                    InlineKeyboardButton("𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥", url="t.me/Ek4mpreetsingh")
                ],[
    ...
    if not files_:
        ...
        try:
            ...
            title = '@MoviesGalaxy120  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@MoviesGalaxy120  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@MoviesGalaxy120  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
                InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
...
async def give_premium_cmd_handler(client, message):
    ...
    if len(message.command) == 3:
        ...
        if seconds > 0:
            ...
            await client.send_message(
                chat_id=user_id,
                text=f"<b>ᴘʀᴇᴍɪᴜᴍ ᴀᴅᴅᴇᴅ ᴛᴏ ʏᴏᴜʀ ᴀᴄᴄᴏᴜɴᴛ ꜰᴏʀ {time} ᴇɴᴊᴏʏ 😀\n\n⌛Rᴇᴍᴀɪɴɪɴɢ ᴛɪᴍᴇ: {remaining_time}\n⏲️ᴇxᴘɪʀʏ ᴅᴀᴛᴇ: {expiry_time}</b>",                
            )
        else:
...
async def check_plans_cmd(client, message):
    ...
    if await db.has_premium_access(user_id):         
        ...
        await message.reply_text(f"**Yᴏᴜʀ ᴘʟᴀɴꜱ ᴅᴇᴛᴀɪʟꜱ ᴀʀᴇ :\n\n⌛Rᴇᴍᴀɪɴɪɴɢ Tɪᴍᴇ : {remaining_time}\n\n⏲️ᴇxᴘɪʀʏ ᴅᴀᴛᴇ: {expiry_time}**")
    else:
        btn = [ 
        ...
        await message.reply_text(f"**😢 Yᴏᴜ Dᴏɴ'ᴛ Hᴀᴠᴇ Aɴʏ Pʀᴇᴍɪᴜᴍ Sᴜʙꜱᴄʀɪᴘᴛɪᴏɴ.\ɴ\ɴ Cʜᴇᴄᴋ Oᴜᴛ Oᴜʀ Pʀᴇᴍɪᴜᴍ /plan**",reply_markup=reply_markup)
